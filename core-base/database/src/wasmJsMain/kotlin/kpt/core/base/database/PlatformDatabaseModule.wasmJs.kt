...
/**
 * WasmJs Koin module that builds the fork's [RoomDatabase] singleton [T].
 *
 * INFRA (owner: template). The WebWorker driver is applied by [AppDatabaseFactory]
 * itself (OPFS persistent when available, in-memory fallback), so only the query
