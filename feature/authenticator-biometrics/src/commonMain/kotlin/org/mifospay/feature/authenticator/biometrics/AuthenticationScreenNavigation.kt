...
const val RE_AUTH_PLATFORM_AUTHENTICATOR_ROUTE = "re_auth_platform_authenticator_route"
...
fun NavController.navigateToReAuthPlatformAuthenticator(navOptions: NavOptions? = null) =
    navigate(RE_AUTH_PLATFORM_AUTHENTICATOR_ROUTE, navOptions)
...
fun NavGraphBuilder.reAuthPlatformAuthenticator(
    onAuthenticationSuccess: () -> Unit,
    onForcedLogOut: () -> Unit,
) {
    composable(route = RE_AUTH_PLATFORM_AUTHENTICATOR_ROUTE) {
        AuthenticationScreen(
            onAuthenticationSuccess,
            onForcedLogOut,
        )
