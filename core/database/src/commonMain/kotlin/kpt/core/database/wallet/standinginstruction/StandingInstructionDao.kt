...
/**
 * Data-access object for the `wallet_standing_instructions` LEDGER table.
 *
 ...
 * ## Sort order
 *
 * Alphabetical by [StandingInstructionEntity.name] (case-insensitive) with
 * `id ASC` as tiebreaker. Names may be blank in the domain (nullable string
 * coalesced to ""); such rows sort to the front of the list.
 */
@Dao
interface StandingInstructionDao {

    /**
     * Observe every cached standing instruction for the given [clientId],
     * alphabetical by name.
     *
     * The returned [Flow] re-emits on every write to
     * `wallet_standing_instructions`. On wasmJs the Store5 factory wraps this
     * in `daoFlow("wallet_standing_instructions") { … }` — see
     * `StandingInstructionStore.kt` — so re-emission fires even when Room 3
     * alpha's async InvalidationTracker fails.
     */
    @Query(
        "SELECT * FROM wallet_standing_instructions " +
            "WHERE clientId = :clientId " +
            "ORDER BY name COLLATE NOCASE ASC, id ASC",
    )
    fun observeByClient(clientId: Long): Flow<List<StandingInstructionEntity>>

    /** Insert-or-replace a batch of rows. Sanctioned merge writer for future delta fetches. */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertAll(entities: List<StandingInstructionEntity>)
     ...
     */
    @Query("DELETE FROM wallet_standing_instructions WHERE clientId = :clientId")
    suspend fun deleteByClient(clientId: Long)

    /** Delete every cached row (StoreCacheManager `clear()` on logout). */
    @Query("DELETE FROM wallet_standing_instructions")
     ...
     */
    @Transaction
    suspend fun replacePage(clientId: Long, entities: List<StandingInstructionEntity>) {
