...
/**
 * Data-access object for the `wallet_saved_cards` LEDGER table.
 *
 ...
 * ## Sort order
 *
 * Descending by `id` (newest card id first) — matches the effective ordering the
 * previous `Flow<List<SavedCard>>` shim returned from the Fineract datatable
 * endpoint (Ktorfit shim preserved server row order; server tends to return
 * newest-first for datatables).
 */
@Dao
interface SavedCardDao {

    /**
     * Observe every cached card for the given [clientId].
     *
     * The returned [Flow] re-emits on every write to `wallet_saved_cards`. On
     * wasmJs the Store5 factory wraps this in `daoFlow("wallet_saved_cards") { … }`
     * — see `SavedCardStore.kt` — so re-emission fires even when Room 3 alpha's
     * async InvalidationTracker fails.
     */
    @Query(
        "SELECT * FROM wallet_saved_cards " +
            "WHERE clientId = :clientId " +
            "ORDER BY id DESC",
    )
    fun observeByClient(clientId: Long): Flow<List<SavedCardEntity>>

    /** Insert-or-replace a batch of rows. Sanctioned merge writer for future delta fetches. */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertAll(entities: List<SavedCardEntity>)
     ...
     */
    @Transaction
    suspend fun replacePage(clientId: Long, entities: List<SavedCardEntity>) {
