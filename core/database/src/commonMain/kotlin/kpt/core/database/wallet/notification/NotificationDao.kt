...
/**
 * Data-access object for the `wallet_notifications` LEDGER table.
 *
 ...
 * ## Sort order
 *
 * Descending by `id` (newest notification id first) — matches the effective
 * ordering the previous `Flow<List<Notification>>` shim returned from the
 * Fineract `notifications/` endpoint (server-issued notification ids grow
 * monotonically).
 */
@Dao
interface NotificationDao {
    ...
    /**
     * Observe every cached notification for the given [clientId], newest id first.
     *
     * The returned [Flow] re-emits on every write to `wallet_notifications`. On
     * wasmJs the Store5 factory wraps this in `daoFlow("wallet_notifications") { … }`
     * — see `NotificationStore.kt` — so re-emission fires even when Room 3
    ...
    fun observeByClient(clientId: Long): Flow<List<NotificationEntity>>
     ...
     */
    @Query("DELETE FROM wallet_notifications WHERE clientId = :clientId")
    suspend fun deleteByClient(clientId: Long)

    /** Delete every cached row (StoreCacheManager `clear()` on logout). */
    @Query("DELETE FROM wallet_notifications")
     ...
     */
    @Transaction
    suspend fun replacePage(clientId: Long, entities: List<NotificationEntity>) {
